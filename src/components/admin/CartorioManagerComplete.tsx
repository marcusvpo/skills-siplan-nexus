...
import { CartorioAccessManager } from './CartorioAccessManager';

const CartorioManagerComplete: React.FC = () => {
  ...
  const [selectedCartorioForAccess, setSelectedCartorioForAccess] = useState<any>(null);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">Gerencie cartórios, usuários e controle de acesso à plataforma</p>
          {session && (
        ...
        {cartorios.length === 0 ? (
          ...
          cartorios.map((cartorio) => (
            <CartorioCard
              ...
              onManageAccess={setSelectedCartorioForAccess}
            />
          ))
        )}
      </div>
      ...
      {selectedCartorioForAccess && (
        <CartorioAccessManager
          cartorioId={selectedCartorioForAccess.id}
          cartorioName={selectedCartorioForAccess.nome}
          isOpen={!!selectedCartorioForAccess}
          onClose={() => setSelectedCartorioForAccess(null)}
        />
      )}
    </div>
  );
...
export default CartorioManagerComplete;
