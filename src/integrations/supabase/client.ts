...
const getSupabaseInstance = () => {
  if (!supabaseInstance) {
    console.log('🔧 [Supabase] Creating optimized client instance with custom storage');
    supabaseInstance = createClient<Database>(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, {
      auth: {
        persistSession: true,
        autoRefreshToken: true,
        detectSessionInUrl: false, // Importante para evitar conflitos
        storage: customCartorioStorage, // ⭐ IMPLEMENTAÇÃO CUSTOM
...
export const ensureSessionHydration = async (): Promise<boolean> => {
  try {
    ...
    if (customToken) {
      console.log('✅ [ensureSessionHydration] Token encontrado no storage customizado');
    }

    // 2. Primeira tentativa: getSession() imediato
    ...
    // 3. Segunda tentativa: forçar refresh da sessão
    console.log('🔍 [ensureSessionHydration] Tentando refresh da sessão...');
    ...
    if (typeof window !== 'undefined') {
      ...
      if (directToken) {
        console.log('🔍 [ensureSessionHydration] Token encontrado diretamente no localStorage');
        try {
